...
pitch_ang_L = wing_angles(1);
pitch_ang_R = wing_angles(1);
roll_ang_L = wing_angles(2);
roll_ang_R = wing_angles(2);

% store the input angles
surf_data{counter}.input.pitch_l = pitch_ang_L;
surf_data{counter}.input.pitch_r = pitch_ang_R;
surf_data{counter}.input.roll_l = roll_ang_L;
surf_data{counter}.input.roll_r = roll_ang_R;

vib_ang = vibration_angle;    

% Body rotation measured by the IMU
R_body = EulerZYX_Fast([br bp by]);
