function plot_righting
    ...
    load dat_righting.mat 
    load dat_righting.mat pe wing pitch roll
...
%% gui controls
    ...
    ff.sl.w = uicontrol('style','slider','string','wing opening', 'tag','tag_sl_w'); 
    ...
    ff.sl.p = uicontrol('style','slider','string','body pitch', 'tag','tag_sl_p');
    ...
    ff.sl.r = uicontrol('style','slider','string','body roll', 'tag','tag_sl_r'); 
...
function update_fig(hObject, event, steps,sh,depvar,pe,robot)
    ...
    switch hObject.String
        case 'wing opening'
            ...
            gg.String=['wing opening = ' str_app num2str(abs(steps(min_idx))) '°'];
        ...
        case 'body roll'
            ...
            gg.String=['body roll = ' str_app num2str(abs(steps(min_idx))) '°'];
            ss = findobj('tag','state_3d');
...
function reset_fig(n0,w0,p0,r0,pe,robot)
    ...
    % slider position
    gg = findobj('tag','tag_sl_w');  gg.Value = w0;
    gg = findobj('tag','tag_sl_r');  gg.Value = r0;
    gg = findobj('tag','tag_sl_p');  gg.Value = p0;
    % title string     
    if(w0<0) str_app='−' ; else str_app=''; end
    gg = findobj('tag','sp_w_sec'); gg.Title.String=['wing opening = '  str_app num2str(abs(w0)) '°'];
    gg = findobj('tag','var_1');     gg.String=['wing opening = '  str_app num2str(abs(w0)) '°']; 
    ...
    ss = findobj('tag','state_cw');    ss.XData = r0;    ss.YData = p0;    ss.ZData = 2000;
    ss = findobj('tag','state_cr');    ss.XData = p0;    ss.YData = w0;    ss.ZData = 2000;
    ss = findobj('tag','state_cp');    ss.XData = r0;    ss.YData = w0;    ss.ZData = 2000;
